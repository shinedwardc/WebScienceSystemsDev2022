...
app.use(express.static(path.join(__dirname, '../frontend/lab8/dist/lab7')));
...
fs.createReadStream('../frontend/lab7/src/assets/imdb_top_1000.csv')
    .pipe(csv())
...
app.get('/csv', function (req,res) {
    const database = client.db('first_database');
    const movies = database.collection('imdb_ratings');
    const list = movies.find({})
    .toArray(function(err,result){
        if (err){
            res.status(400).send("Error fetching the documents!");
